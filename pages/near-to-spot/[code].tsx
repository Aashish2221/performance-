...
export default function NearToSpot({
 ...
 useEffect(() => {
  setPageNumber(1); // Reset pageNumber to 1 immediately when searchKeyword changes
  setHasMore(true);
  setProducts([]);
}, [searchKeyword]);
...
useEffect(()=>{
  ...
  try {
    const response = await fetch(
      `${process.env.BASE_URL}/api/BestBullionDeals/GetHomePageProductsByLocation_SpotChart?GetBy=NearToSpot&Size=12&Pagenumber=1&MetalType=${searchKeyword}`
    );
    if (!response.ok) {
